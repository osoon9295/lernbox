...
이 프로젝트는 **AI 도구(Claude Code)를 활용한 학습 프로젝트**입니다. 애플리케이션 구현의 상당 부분은 AI로 생성했고, 제 초점은 **생성된 코드와 테스트를 QA/SDET 관점에서 이해하고 검증하는 것**에 있었습니다.

**직접 작성하고 디버깅한 것**

- **E2E 자동화** — Playwright + Page Object Model로 로그인 흐름을 작성. auto-waiting, 크로스 브라우저(Chromium·Firefox·WebKit) 교차 검증.
- **CI 파이프라인** — GitHub Actions에서 임시 PostgreSQL을 프로비저닝해 E2E를 자동 실행하는 워크플로우를 구성하고, 그린이 될 때까지 직접 디버깅.
- **크로스 브라우저 버그 2건** — WebKit에서만 재현되는 로그인 실패를 trace·실패 아티팩트로 추적해 원인을 규명하고 해결. (아래 [트러블슈팅](#트러블슈팅))

**읽고 이해하고 검증한 것**

- AI가 생성한 **단위·통합·컴포넌트 테스트**를 세 층 모두 코드 단위로 정독.
- 각 층을 **의도적으로 고장내(mutation)** "각 테스트가 무엇을 지키는지"를 확인. (아래 [AI 생성 테스트 검증](#ai-생성-테스트-검증))

> AI가 생성한 코드를 그대로 신뢰하지 않고 읽고 검증하는 것은, 앞으로 QA가 맡게 될 핵심 역할이라고 생각합니다. 이 저장소는 그 작업을 연습하고 기록한 공간입니다.
